...
- Higher attack surface. Ubuntu server is a full linux distribution with a shell, multiple packages and services enabled by default. It is harder to garuntee that the system running kubernetes hasn't been tampered with.
...
Generally, this depends on the machine you are using to run Talos. For AMD-based machines, you can select all of the amd extensions.

If you plan on running virtualized github runners, you will need `siderolabs/binfmt-misc` as it is a dependency.

If you have a Intel CPU, you will need the intel-ucode extension.

The zfs extension is incompatiable with the nvidia extension at the moment.

The nut client & zfs extensions require additional configurations
