...
update4=$(curl -s -X PUT \
    ...
    -H "X-Auth-Key: $CLOUDFLARE_API_KEY" \
    -H "Content-Type: application/json" \
    --data "{\"id\":\"$CLOUDFLARE_ZONE_ID\",\"type\":\"A\",\"proxied\":true,\"name\":\"$CLOUDFLARE_RECORD_NAME\",\"content\":\"$ip4\"}" \
)
...
if echo "$update4" | grep -q '\"success\":false'; then
    ...
    printf "%s - Success - IP Address '%s' has been updated on %s" "$(date -u)" "$ip4" "$CLOUDFLARE_RECORD_NAME.$CLOUDFLARE_ZONE_ID"
    exit 0
fi
